...
void caffe_gpu_interp2(const int channels,
  ...
  caffe_gpu_interp2_kernel<Dtype, packed>
    <<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>(
      num_kernels, rheight, rwidth, channels, data1, x1, y1, height1, width1,
      Height1, Width1, data2, x2, y2, height2, width2, Height2, Width2);
...
void caffe_gpu_interp2_backward(const int channels,
  ...
  const float rwidth =
    (width2 > 1) ? static_cast<float>(width1 - 1) / (width2 - 1) : 0.f;
  const int num_kernels = height2 * width2;
  caffe_gpu_interp2_kernel_backward<Dtype, packed>
    <<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>(
      num_kernels, rheight, rwidth, channels, data1, x1, y1, height1, width1,
      Height1, Width1, data2, x2, y2, height2, width2, Height2, Width2);
  CUDA_POST_KERNEL_CHECK;
...
template <typename Dtype, bool packed>
__global__ void caffe_gpu_pyramid2_kernel(const int n,
                                          const int channels,
                                          const Dtype *data1,
                                          const int height1,
                                          const int width1,
...
void caffe_gpu_pyramid2(const int channels,
  ...
  for (int l = 0; l < levels; ++l) {
    ...
    const int num_kernels = height2 * width2;
    caffe_gpu_pyramid2_kernel<Dtype, packed>
      <<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>(
...
// Explicit instances
template void caffe_gpu_interp2<float, false>(const int,
                                              const float *,
                                              const int,
                                              const int,
                                              const int,
                                              ...
                                              const int,
                                              const int);
template void caffe_gpu_interp2<float, true>(const int,
                                             const float *,
                                             const int,
                                             const int,
                                             const int,
                                             ...
                                             const int,
                                             const int);
template void caffe_gpu_interp2<double, false>(const int,
                                               const double *,
                                               const int,
                                               const int,
                                               const int,
                                               ...
                                               const int,
                                               const int);
template void caffe_gpu_interp2<double, true>(const int,
                                              const double *,
                                              const int,
                                              const int,
                                              const int,
...
template void caffe_gpu_interp2_backward<float, false>(const int,
                                                       float *,
                                                       const int,
                                                       const int,
                                                       const int,
                                                       ...
                                                       const int,
                                                       const int);
template void caffe_gpu_interp2_backward<double, false>(const int,
                                                        double *,
                                                        const int,
                                                        const int,
                                                        const int,
...
template void caffe_gpu_pyramid2<float, false>(
  const int, const float *, const int, const int, float *, const int);
template void caffe_gpu_pyramid2<float, true>(
  const int, const float *, const int, const int, float *, const int);
template void caffe_gpu_pyramid2<double, false>(
  const int, const double *, const int, const int, double *, const int);
template void caffe_gpu_pyramid2<double, true>(
  const int, const double *, const int, const int, double *, const int);
