 ...
 * @Last Modified time: 2019-12-19 22:05:34
...
std::vector<torch::Tensor> cubic_feature_sampling_forward(
  int scale, torch::Tensor ptcloud, torch::Tensor cubic_features) {
  CHECK_INPUT(ptcloud);
  CHECK_INPUT(cubic_features);

  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  return cubic_feature_sampling_cuda_forward(scale, ptcloud, cubic_features,
                                             stream);
}

std::vector<torch::Tensor> cubic_feature_sampling_backward(
  int scale, torch::Tensor grad_point_features, torch::Tensor grid_pt_indexes) {
  CHECK_INPUT(grad_point_features);
  CHECK_INPUT(grid_pt_indexes);

  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  return cubic_feature_sampling_cuda_backward(scale, grad_point_features,
